...
func (_q *PluginQuery) QueryVersions() *PluginVersionQuery {
	query := (&PluginVersionClient{config: _q.config}).Query()
...
func (_q *PluginQuery) WithVersions(opts ...func(*PluginVersionQuery)) *PluginQuery {
	query := (&PluginVersionClient{config: _q.config}).Query()
...
//
//	client.Plugin.Query().
//		GroupBy(plugin.FieldNamespaceID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Plugin.Query().
//		Select(plugin.FieldNamespaceID).
//		Scan(ctx, &v)
...
func (_q *PluginQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Plugin, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, plugin.ForeignKeys...)
	}
	...
	if query := _q.withVersions; query != nil {
		if err := _q.loadVersions(ctx, query, nodes,
			func(n *Plugin) { n.Edges.Versions = []*PluginVersion{} },
			func(n *Plugin, e *PluginVersion) { n.Edges.Versions = append(n.Edges.Versions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PluginQuery) loadVersions(ctx context.Context, query *PluginVersionQuery, nodes []*Plugin, init func(*Plugin), assign func(*Plugin, *PluginVersion)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(pluginversion.FieldPluginID)
	}
	query.Where(predicate.PluginVersion(func(s *sql.Selector) {
