...
func (_q *UserQuery) QueryNamespaces() *NamespaceQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withNamespaces:    _q.withNamespaces.Clone(),
...
func (_q *UserQuery) WithNamespaces(opts ...func(*NamespaceQuery)) *UserQuery {
	query := (&NamespaceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withNamespaces = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withNamespaces; query != nil {
		if err := _q.loadNamespaces(ctx, query, nodes,
			func(n *User) { n.Edges.Namespaces = []*Namespace{} },
			func(n *User, e *Namespace) { n.Edges.Namespaces = append(n.Edges.Namespaces, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadNamespaces(ctx context.Context, query *NamespaceQuery, nodes []*User, init func(*User), assign func(*User, *Namespace)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
